...
RMSP250:
    Be topdown sr:
        Last batch train loss (using only train_net): 0.0377015
        mse from real: 0.260139928988
    
    ,kai 1000:
        Last batch train loss (using only train_net): 0.146612
        mse from real: 1.78300929211
    ...
    hidden_size_sr_topdown = 3:
        Last batch train loss (using only train_net): 0.0412188
        mse from real: 0.215231805361
        
        ,kai 1000
        Last batch train loss (using only train_net): 0.0500894
        mse from real: 0.267889746168

        ,kai 2500 (seed=10)
        Last batch train loss (using only train_net): 0.0597788
        mse from real: 0.103182051731
...
class Network(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate, tau):
        ...
        self.loss = tf.reduce_mean((tf.square(self.curr_state + GAMMA*self.target_out - self.out)) + 0.05*tf.nn.relu(-self.out))
