"""
setting w[5]=1 in 6x6 environment, should produce solution, similar to Exact-SR-M_rowOF6.py 
"""
...
class Network(object):
    ...
    def create_network(self, scope):
        ...
        with tf.variable_scope(scope):
            W_s = tf.Variable(tf.truncated_normal([self.state_dim, hidden_size]))
        ...
        sr_in = selu(tf.matmul(inputs, W_s)+ tf.matmul(action, W_a) + b_sr)
...
with tf.Session() as sess:
    ...
    print(np.round(np.reshape(test_state + GAMMA*SR2, (6, 6)),2)) #going down from 0
    print('')
    print(np.round(np.reshape(SR, (6, 6)), 2)) #going right from 0

    #print(loss)
    loss0 = net.test_loss(test_state + GAMMA*SR2, SR)
    print (loss0)
    acc, loss = net.train(np.reshape(s_batch[16], (1, 36)),
                          np.reshape(a_batch[16], (1, 4)),
                          np.reshape(s2_batch[16], (1, 36)),
                          np.reshape(a2_batch[16], (1, 4)))
